  ...
  try {
    ...
    if (strongPasswordRegex.test(password)) {
      return res.status(400).json({
        message:
          "Password must be more than 8 chras, upper case, lower case, number and special character",
      });
    }
    ...
    if (!emailPattern.test(email)) {
      return res
        .status(400)
        .json({ message: "email should be valid, contain @ and .com" });
    }
...
export const getAllUsers = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "users get successfully", users: allUsers });
  } catch (err) {
    return res
      .status(500)
      .json({ message: "Internal server error in get all users!" });
  }
};
